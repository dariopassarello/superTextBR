...
public class Fight
{
    ...
    private void updateDiscipline(int round)
    {
        ...
        for (Player p : this.players)
        {
            ...
            if (i == 0)
                p.setDiscipline(p.getDiscipline() + Fight.FIGHT_STARTER_DISCIPLINE_BONUS); //Il giocatore che attaca per primo fugge piu difficilmente
            if (p.isDisarmed())
                p.setDiscipline(p.getDiscipline() + Fight.DISARMED_ESCAPE_BONUS); //Se il giocatore è disarmato fugge piu facikente
        ...
        {
            if (p.getHP() >= maxHP)
                p.setDiscipline(p.getDiscipline() + Fight.HEALTIEST_DISCIPLINE_BONUS); //Aggiungi bonus per il giocatore con piu HP (fugge + difficilmente)
        }
    ...
    public int nextHit()
    {
        ...
        int prevLevel = this.activePlayer.getLevel();
        if (this.players.length > 1)
        {
            if (round > 0 && activePlayerNum == 0) this.shuffleArray();
            if (this.turn > 1)
            {
            ...
            } else
            {
                ...
                } else
                {
                    ...
                    if (!(RandomManager.isInRandomRange(0, 100, 0, Fight.USE_WEAKEST_WEAPON_CHANCE))
                            || worstWeapon.getLevel() == 0) weaponUsed = bestWeapon;
                    else weaponUsed = worstWeapon;
                }
            }
            ...
            //Crea il resoconto del handlers per il narratore
            if (targetPlayer.isAlive())
            {
                this.status = STATUS_STILL_FIGHTING;
            } else if (!targetPlayer.isAlive() && this.turn > 0)
            {
